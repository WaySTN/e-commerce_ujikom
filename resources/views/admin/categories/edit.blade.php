<x-admin-layout>
    ...
    <div class="max-w-xl mx-auto bg-slate-950 border border-slate-800 rounded-3xl p-4 sm:p-8 shadow-xl">
        <h3 class="font-bold text-base sm:text-lg text-white mb-6 border-b border-slate-800 pb-3">Form Edit Kategori</h3>

        <form method="POST" action="{{ route('admin.kategori.update', $category->id) }}" class="space-y-5">
            @csrf
            @method('PUT')
